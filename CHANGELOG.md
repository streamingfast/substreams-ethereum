...
- **Breaking**: protobuf encoding and decoding moved from [prost](https://github.com/tokio-rs/prost) to
  ...
  Event decoders generated by `Abigen` are now generic over the log representation, so the same
  decoder works with an owned `Log`, a `LogView`, or one of buffa's borrowed lazy views:
  ...
  ABI decoding itself is unchanged; only the log's type is now generic.

  Types generated by buffa differ from prost in ways that affect module code:

  | | prost | buffa |
  |---|---|---|
  | enum field (`status`, `call_type`, `reason`, …) | `i32` | `EnumValue<E>`, compare with `tx.status == TransactionTraceStatus::Succeeded` |
  | singular message field (`header`, `receipt`, …) | `Option<Box<T>>` | `MessageField<T>`, derefs to a default instance, `.as_option()` for interop |
  | `Log.block_index` | `block_index` | `blockIndex`; `LogView::block_index()` is unchanged |

  Two behaviour changes that do **not** produce compile errors:

  - `Block::timestamp()` and `timestamp_seconds()` previously panicked on a block with no header.
    They now return a default timestamp, because `MessageField` derefs to a default instance.
  - `TransactionTrace::receipt()` likewise no longer panics on a transaction with no receipt.

- **Breaking**: `Event::match_log` and `Event::decode` are generic over `L: LogLike`. A hand-written
  `impl Event` needs updating; generated code is regenerated by `Abigen` and needs no change.
  `Function::match_and_decode` is unchanged: call decoding is not generic yet.

  The failed-decode log line now reports the log's index within its transaction rather than its
  index within the block, which is what `LogLike` exposes.

- **Breaking**: `Call.executed_code` is a plain `bool` rather than `Option<bool>`. The checked-in
  generated code had fallen behind the schema, which declares `bool executed_code = 15`.

- **Breaking**: the block model is regenerated from a newer `sf.ethereum.type.v2` schema, which adds
  `BlockHeader.slot_number` and makes `BlockHeader.parent_beacon_root` and `BlockHeader.requests_hash`
  optional.

### Added

- `LogLike`, the log fields ABI decoding reads, implemented for the owned `Log`, `LogView` and
  buffa's `LogLazyView`.
- Block accessors over buffa's lazy views: `transactions()`, `receipts()`, `logs()`, `calls()` and
  `TransactionTraceLazyView::logs_with_calls()`. Because a lazy view defers validation to field
  access, each has a `try_` twin that surfaces decode errors where the plain one skips them.

### Changed (internal)

- `pb` types are generated by the `buf.build/anthropics/buffa` plugin rather than the prost one.
  They stay checked in and are regenerated with `buf generate`, which now reads the module
  and exclusions from `buf.gen.yaml` rather than needing the `gen.sh` wrapper.

- `pb::sf::ethereum::transform::v1` is no longer exported. Generation has always excluded that
  path, so the checked-in file was stale output that regeneration would not have produced.
...
- `cargo test` works again. `.cargo/config.toml` set `wasm32-unknown-unknown` as the default target
  ...
  CI already named its targets explicitly and was unaffected.
